...
- [ ] **Still owed, and it belongs to the session that builds.** The notices on disk were generated from a real build on 2026-09-21 and are committed, and they are internally consistent: NOTICE.md section 7 says 48 Python packages and 15 native libraries, and `THIRD_PARTY_NOTICES.md` lists exactly 15 native entries (counted 2026-09-21). But the artifact that will ship has not been rebuilt yet, so this box cannot honestly be ticked by a session that did not build. **The original instruction:** re-run the generator in section 7 against the release build, not a stale one, and commit the result.
