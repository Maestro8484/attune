...
**Corresponding source for the bundled FFmpeg.** The binary shipped is an unmodified build published by gyan.dev, which states its builds are GPLv3 and links to the exact FFmpeg commit each one was built from ([gyan.dev/ffmpeg/builds](https://www.gyan.dev/ffmpeg/builds/), source at [ffmpeg.org](https://ffmpeg.org/download.html)). Attune does not patch it, wrap it, or link against it; it is launched as a separate program. The source for that build is FFmpeg's own, at the release gyan.dev names for it, and the exact versions of the libraries compiled into it are listed in the README.txt inside gyan.dev's own download of the same build. Attune does not keep a copy of those sources. If the links above ever stop working, open an issue on this repository and we will help you find the same sources.
...
**The optional learned head is the author's own weights.** `src/models/metric_head.onnx` and `src/models/learned_norm.json` were produced on the author's machine over a pool of 17,020 of his own tracks, with MusicIP's rankings as the training target. The head carries the date 2026-07-09 in `src/engine.py`; the statistics file beside it is dated 2026-07-18. Section 4 says what that means. It is not used unless the engine is set to `learned`. **Both files are offered under the same MIT licence as the code** ([LICENSE](LICENSE)), as the author's own work. That is the licence; section 4 is the disclosure of where the judgments inside the head came from, and the disclosure stands beside the licence rather than being replaced by it.
