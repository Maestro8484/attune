...
Attune is a modern retrofit of the programs that made a PC the center of a music collection. The jukeboxes of the late nineties that ripped CDs and burned them back. The players that would fill a playlist for you. The analyzers that could hear that two songs sounded alike. Most of those programs are dead, none of them were open-sourced, and the good ideas in them went to the grave with the companies.

Attune takes the best of those ideas, a playlist built from the sound of one song, and brings it into the present. It's a desktop app that runs on your own machine, analyzes your files once, builds sounds-like playlists instantly, pushes them to Plex, and copies them to a USB stick for the car. No account. No server. Nothing that can go dark.

The idea I wanted back most was MusicIP Mixer's. You point at a song and it hands you a playlist of songs that actually sound like it. Not "people who liked this also liked", not genre tags. The sound. I used it for years after the company behind it dissolved in 2008, because nothing else did the job. Apple's Genius did something close inside iTunes, but it worked by comparing your library against everyone else's on Apple's servers, and it only worked while Apple felt like running them.
...
The thing that finally pushed me into building was not nostalgia. It was the six steps it took to get a MusicIP mix into my car. Its player hooks were built for the players of the 2000s (Winamp, iTunes, Windows Media Player), so on my machine it was: make the mix, save an m3u, import that into MusicBee, make a MusicBee playlist, export it to a folder, copy the folder to a USB stick. Every time.

So I built Attune. Then the project taught me two things I didn't want to learn, both covered below.
...
| iTunes Genius | 2008 onward | One button, one seed song, a playlist. Built from everyone's libraries on Apple's servers rather than from the audio. | [MIT Technology Review, 2010](https://www.technologyreview.com/2010/06/02/91325/how-itunes-genius-really-works/), [Apple's own terms](https://www.apple.com/legal/internet-services/itunes/us/genius.html) |
| Squeezebox and the Lyrion server | 2000s onward | The community that kept MusicIP alive after the company, by driving it through its local interface. Still the best documentation of that interface. | [Lyrion wiki](https://wiki.lyrion.org/index.php/Integrating_MusicIP_with_SqueezeCenter), [lms-mipmixer](https://github.com/CDrummond/lms-mipmixer) |
...
Basically, the whole engine is one similarity function and five lines of logic around it. I got that wrong first, published the wrong answer to myself, and had to overturn it. I also got the range of the "style" dial wrong: it goes to roughly 845, not 100, so my whole first sweep covered about an eighth of it.
...
That listening test is five tracks, and it was only partly blind: the three outside engines, MusicIP among them, were shuffled and unlabelled, while my own engines were labelled as mine. One of its five rounds has a labelling note I never resolved. It's a pilot, not a study. I think the homemade engine holds its own against the original, and I wouldn't say more than that from five tracks.
...
The lineage: The jukeboxes and mixers of the 2000s made the PC the center of a music collection, then their companies died and took the ideas with them. Attune brings the best one back, and adds the two things a collection needs today: a push to Plex and a copy to the USB stick in the car.
...
Attune is a Windows desktop app that analyzes your music collection once, then builds playlists from the sound of a single seed song. Not from tags, not from what other people played. From the audio.

The lineage matters. Between about 1997 and 2008 a run of PC programs made the computer the center of a music collection: MusicMatch Jukebox ripped and burned, MoodLogic profiled by mood, and Predixis's MusicMagic Mixer (later MusicIP) analyzed the audio itself and built cross-genre mixes from one song. All of them are dead. None were open-sourced. I kept using MusicIP for years after its company dissolved, because nothing replaced it. Attune is that idea rebuilt for now.

What it does:

- Analyzes each track once with a music-trained neural model plus a set of plain acoustic descriptors (timbre, tempo, loudness, brightness). Everything is stored in one local database you can open.
- Builds a sounds-like playlist from any seed song instantly, with a Similarity knob and a Variety knob.
...
Attune is a Windows desktop app. It analyzes your library once (a music-trained neural model plus plain acoustic features, all stored in one local SQLite file), then builds a playlist from any seed song instantly. Similarity and Variety knobs, like the original.
...
- The listening model runs on CPU through ONNX Runtime. No CUDA, no torch in the installed app.
...
- If you still have MusicIP running, Attune detects it and can use it as the engine. Otherwise you get Attune's own, which held its own against the real thing in a small listening test.

One thing I found out on the way, by measuring the old program through its own local API: MusicIP's variety slider was never "diversity logic". It ranks your whole library once and flips a biased coin down the list. The bias is exactly one over one-plus-the-setting. Twenty years of forum lore about "journeys" was a coin flip.
...
Two things worth getting right. Attune does not need MusicIP installed, at all. The default engine setting is "auto", which uses MusicIP only if it finds one already running on the machine, and Attune's own engine otherwise. Almost nobody has MusicIP, so almost everybody gets Attune's engine, and the analysis never touches MusicIP either way. And the built application is under the GPL because of what's bundled inside it, while the source is MIT. `NOTICE.md` explains that properly.
